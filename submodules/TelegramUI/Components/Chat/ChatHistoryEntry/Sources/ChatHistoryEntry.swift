...
public enum ChatHistoryEntry: Identifiable, Comparable {
    ...
    case AdFoldedEntry(count: Int, index: EngineMessage.Index, presentationData: ChatPresentationData, isExpanded: Bool)
    ...
    public var stableId: UInt64 {
        ...
        case let .AdFoldedEntry(count, index, _, _):
            return UInt64(bitPattern: Int64(index.id.id)) | ((UInt64(8) << 40)) + UInt64(count)
        }
    }
    ...
    public var index: EngineMessage.Index {
        ...
        case let .AdFoldedEntry(_, index, _, _):
            return index
        }
    }
    ...
    public static func ==(lhs: ChatHistoryEntry, rhs: ChatHistoryEntry) -> Bool {
        switch lhs {
            ...
            case let .AdFoldedEntry(lhsCount, lhsIndex, lhsPresentationData, lhsIsExpanded):
                if case let .AdFoldedEntry(rhsCount, rhsIndex, rhsPresentationData, rhsIsExpanded) = rhs, lhsCount == rhsCount, lhsIndex == rhsIndex, lhsPresentationData === rhsPresentationData, lhsIsExpanded == rhsIsExpanded {
                    return true
                } else {
                    return false
                }
