...
private func mappedInsertEntries(context: AccountContext, chatLocation: ChatLocation, associatedData: ChatMessageItemAssociatedData, controllerInteraction: ChatControllerInteraction, mode: ChatHistoryListMode, lastHeaderId: Int64, isSavedMusic: Bool, canReorder: Bool, entries: [ChatHistoryViewTransitionInsertEntry]) -> [ListViewInsertItem] {
    ...
    return entries.map { entry -> ListViewInsertItem in
        switch entry.entry {
            ...
            case let .AdFoldedEntry(count, index, presentationData, isExpanded):
                return ListViewInsertItem(index: entry.index, previousIndex: entry.previousIndex, item: ChatAdFoldedItem(count: count, index: index, presentationData: presentationData, controllerInteraction: controllerInteraction, isExpanded: isExpanded, toggleExpand: { [weak controllerInteraction] in
                    controllerInteraction?.toggleAdFold(foldId: index.id.id)
                }), directionHint: entry.directionHint)
        }
    }
}
...
private func mappedUpdateEntries(context: AccountContext, chatLocation: ChatLocation, associatedData: ChatMessageItemAssociatedData, controllerInteraction: ChatControllerInteraction, mode: ChatHistoryListMode, lastHeaderId: Int64, isSavedMusic: Bool, canReorder: Bool, entries: [ChatHistoryViewTransitionUpdateEntry]) -> [ListViewUpdateItem] {
    ...
    return entries.map { entry -> ListViewUpdateItem in
        switch entry.entry {
            ...
            case let .AdFoldedEntry(count, index, presentationData, isExpanded):
                return ListViewUpdateItem(index: entry.index, previousIndex: entry.previousIndex, item: ChatAdFoldedItem(count: count, index: index, presentationData: presentationData, controllerInteraction: controllerInteraction, isExpanded: isExpanded, toggleExpand: { [weak controllerInteraction] in
                    controllerInteraction?.toggleAdFold(foldId: index.id.id)
                }), directionHint: entry.directionHint)
        }
    }
...
public final class ChatHistoryListNodeImpl: ListView, ChatHistoryNode, ChatHistoryListNode {
    ...
    private let expandedAdFoldIdsPromise = ValuePromise<Set<Int32>>(Set())
    var expandedAdFoldIds: Set<Int32> = Set() {
        didSet {
            if self.expandedAdFoldIds != oldValue {
                self.expandedAdFoldIdsPromise.set(self.expandedAdFoldIds)
            }
        }
    }

    func toggleAdFold(foldId: Int32) {
        if self.expandedAdFoldIds.contains(foldId) {
            self.expandedAdFoldIds.remove(foldId)
        } else {
            self.expandedAdFoldIds.insert(foldId)
        }
    }
    ...
    private func beginChatHistoryTransitions(resetScrolling: Bool, switchedToAnotherSource: Bool) {
        ...
        let historyViewTransitionDisposable = (combineLatest(queue: messageViewQueue,
            ...
            case let .HistoryView(view, type, scrollPosition, flashIndicators, originalScrollPosition, data, id):
                ...
                let previousChatHistoryEntriesForViewState: ChatHistoryEntriesForViewState = chatHistoryEntriesForViewState.modify { state in
                    var state = state
                    state.expandedAdFoldIds = self.expandedAdFoldIds
                    return state
                }
