    ...
    return entries.map { entry -> ListViewInsertItem in
        switch entry.entry {
            ...
            case let .AdFoldedEntry(count, index, presentationData, isExpanded):
                let item = ChatAdFoldedItem(count: count, index: index, presentationData: presentationData, controllerInteraction: controllerInteraction, isExpanded: isExpanded, toggleExpand: { [weak controllerInteraction] in
                    controllerInteraction?.toggleAdFold(index.id.id)
                })
                return ListViewInsertItem(index: entry.index, previousIndex: entry.previousIndex, item: item, directionHint: entry.directionHint)
        }
    }
