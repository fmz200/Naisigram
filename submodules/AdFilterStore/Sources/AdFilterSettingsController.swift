...
public final class AdFilterSettingsController: ViewController {
    ...
    public init(context: AccountContext, peerId: EnginePeer.Id?, peerTitle: String, isGlobalMode: Bool = false, didSetRules: (([AdFilterRule]) -> Void)? = nil) {
        ...
        let navigationBarTheme = NavigationBarTheme(
            ...
            buttonColor: theme.list.itemAccentColor,
            disabledButtonColor: theme.list.itemSecondaryTextColor,
            primaryTextColor: theme.list.itemPrimaryTextColor,
            backgroundColor: theme.list.plainBackgroundColor,
            enableBackgroundBlur: false,
            separatorColor: theme.list.itemPlainSeparatorColor,
            badgeBackgroundColor: theme.list.itemAccentColor,
            badgeStrokeColor: theme.list.itemPlainSeparatorColor,
            badgeTextColor: theme.list.plainBackgroundColor,
            accentButtonColor: theme.list.itemAccentColor,
            accentDisabledButtonColor: theme.list.itemSecondaryTextColor,
            accentForegroundColor: theme.list.plainBackgroundColor
        )
        let navigationBarStrings = NavigationBarStrings(
            back: "Back",
            close: "Close"
        )
    ...
    private func setupUI() {
        ...
        if self.isGlobalMode {
            ...
            self.descriptionNode.attributedText = NSAttributedString(
                string: "Rules added here apply to all channels and groups.",
            ...
            self.descriptionNode.attributedText = NSAttributedString(
                string: "Enter a regular expression. Messages matching the pattern will be collapsed.",
                font: Font.regular(14),
                textColor: theme.list.itemSecondaryTextColor
            )
        ...
        self.inputBackgroundNode.backgroundColor = theme.list.plainBackgroundColor
        ...
        for rule in self.rules {
            let node = AdFilterRuleNode(
                ...
                onToggle: { [weak self] in
                    // Find the peerId that owns this rule
                    let allSettings = AdFilterStore.shared.currentSettings()
                    for config in allSettings.configs {
                        if config.rules.contains(where: { $0.id == rule.id }) {
                            AdFilterStore.shared.toggleRule(id: rule.id, forPeer: config.peerId)
                            break
                        }
                    }
                    self?.loadRules()
                ...
                onDelete: { [weak self] in
                    ...
                    for config in allSettings.configs {
                        if config.rules.contains(where: { $0.id == rule.id }) {
                            AdFilterStore.shared.removeRule(id: rule.id, forPeer: config.peerId)
                            break
                        }
                    }
                    self?.loadRules()
...
private final class AdFilterRuleNode: ASDisplayNode {
    ...
    init(rule: AdFilterRule, theme: PresentationTheme, onToggle: @escaping () -> Void, onDelete: @escaping () -> Void) {
        self.rule = rule
